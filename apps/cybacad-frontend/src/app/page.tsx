...
// Define the shape of your course data matching the Backend
interface Course {
  id: string;
  title: string;
  description: string;
  instructor?: string;
  thumbnail?: string; // AI generates this now
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      try {
        // Fetches from the new route we just added
        ...
        setError("Could not connect to the Learning Engine.");
      } finally {
        setLoading(false);
      }
    }
      ...
      <div id="courses" className="max-w-7xl mx-auto py-24 px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2 max-w-xl">
              Hand-picked interactive labs generated by AI to take you from beginner to professional.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
           ...
           // ERROR STATE
           <div className="text-center py-20 bg-red-900/10 border border-red-900/50 rounded-2xl">
              <h3 className="text-red-400 font-bold text-lg mb-2">Connection Failed</h3>
              <p className="text-gray-400">{error}</p>
              <p className="text-xs text-gray-500 mt-2">Is the backend running on port 5000?</p>
           </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.length > 0 ? (
              ...
              courses.map((course) => (
                <Link 
                  key={course.id} 
                  href={`/learn/${course.id}`} // ✅ Links to the lesson player
                ...
                >
                  <div className="h-52 bg-gray-800 relative overflow-hidden">
                      ...
                      {course.thumbnail ? (
                        ...
                        <div className="absolute inset-0 bg-gradient-to-tr from-gray-900 via-gray-800 to-teal-900/20" />
                      )}
                      ...
                      <div className="absolute bottom-4 right-4 px-3 py-1 bg-black/60 backdrop-blur rounded-full text-xs font-bold text-gray-300 border border-gray-700 flex items-center gap-1">
                         <Zap className="w-3 h-3 text-yellow-400" /> AI Generated
                      </div>
                  </div>
              ...
              <div className="col-span-full py-20 text-center bg-gray-900/30 rounded-2xl border border-dashed border-gray-800">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  Go to the <Link href="/admin" className="text-teal-400 underline">Admin Portal</Link> to generate your first AI course!
                </p>
              </div>
            )}
          </div>
